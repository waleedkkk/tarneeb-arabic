...
import type { AiLevel, AiStyle, Card, MatchState, Seat, Suit } from "./types";
...
export function estimateAiBid(hand: Card[], level: AiLevel, style: AiStyle): number {
  ...
  const raw = 6 + Math.floor((strongestSuit + outsideControls + styleOffset(style) + skillOffset) / 3.7);
  return Math.max(7, Math.min(13, raw));
}

export function chooseAiBid(hand: Card[], highestBid: number | null, level: AiLevel, style: AiStyle): number | null {
  const estimate = estimateAiBid(hand, level, style);
  if (highestBid === null) {
...
export function chooseAiTrump(hand: Card[], level: AiLevel, style: AiStyle): Suit {
  return [...SUITS].sort((left, right) => {
    ...
    const leftScore = leftCards.length * (1.1 + levelWeight) + aiSuitStrength(hand, left) + (style === "مبادر" && leftCards.some((card) => card.rank === 14) ? 0.4 : 0);
    const rightScore = rightCards.length * (1.1 + levelWeight) + aiSuitStrength(hand, right) + (style === "مبادر" && rightCards.some((card) => card.rank === 14) ? 0.4 : 0);
...
function chooseLeadCard(state: MatchState, playerId: Seat, playable: Card[], level: AiLevel, style: AiStyle) {
  if (level === "مبتدئ") return [...playable].sort(byLowestRank)[0];
  ...
  const scored = pool.map((card) => {
    ...
    const exposedVoidPenalty = level === "خبير" && opponentsHaveShownVoid(state, team, card.suit) ? 4 : 0;
    const rankIntent = style === "مبادر" ? card.rank * 0.12 : style === "حذر" ? -card.rank * 0.04 : 0;
  ...
  return [...bestSuitCards].sort(style === "مبادر" ? byHighestRank : byLowestRank)[0];
}

export function chooseAiCard(state: MatchState, playerId: 1 | 2 | 3, level: AiLevel, style: AiStyle): Card {
  const hand = state.players[playerId].hand;
  const playable = legalCards(hand, state.trick);
  const leadSuit = state.trick.leadSuit;
  const trumpSuit = state.bidding.trumpSuit!;
  if (!leadSuit || state.trick.plays.length === 0) return chooseLeadCard(state, playerId, playable, level, style);
  ...
  if (partnerIsWinning && level !== "مبتدئ") {
  ...
  if (winningOptions.length > 0) {
    const ordered = [...winningOptions].sort(style === "مبادر" && level !== "مبتدئ" ? byHighestRank : byLowestRank);
    return ordered[0];
  }
  if (level === "خبير") {
    ...
    return [...(discardPool.length > 0 ? discardPool : playable)].sort(style === "حذر" ? byLowestRank : byHighestRank)[0];
  }
